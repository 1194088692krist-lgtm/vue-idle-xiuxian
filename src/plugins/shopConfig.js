...
export const SKIN_SHOP_CONFIG = {
  ...
  // 用户要求：彻底屏蔽所有男性角色。以下清单基于原始立绘（char_XXX.jpg）视觉性别确认，
  // 含皮肤数 >=6（进入候选池）的男性角色，以及皮肤数 <6（已被 minSkinCount 过滤，保留以防数据变更）的男性角色。
  excludedCharacters: [
    // 以下角色皮肤数 >=6，会进入候选池，必须排除（原始立绘确认为男性）
    'char_007', // 熊娇娇（名字误导，立绘为男性）
    'char_008', // 夜鸢（立绘为男性）
    'char_013', // 血罗刹（立绘为男性）
    'char_014', // 逆鳞（用户确认男性）
    'char_015', // 连珠（立绘为男性）
