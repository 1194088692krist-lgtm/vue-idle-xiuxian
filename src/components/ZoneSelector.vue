<template>
  <div class="zone-selector">
      ...
      <div v-if="!combatState.inCombat" class="idle-section">
        ...
        <div v-if="isIdling && idleDashboard" class="idle-dashboard">
          ...
          <div v-if="activePillBuffList.length" class="dash-pill-buffs">
            ...
            <div v-for="buff in activePillBuffList" :key="buff.type" class="dash-pill-buff-item">
    ...
    <!-- 击杀BOSS立绘演出由 App.vue 全局挂载（单一实例），此处不再重复挂载
         旧实现同时挂载两份会导致两组随机文案同时 teleport 到 body 叠加显示 -->
  </div>
</template>
