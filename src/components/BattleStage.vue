      ...
      <div class="enemy-side">
        <div class="fighter enemy" :class="getEnemyClasses()">
          ...
          <div class="fighter-info">
            ...
            <div class="hp-text">{{ formatNumber((encounter.inProgress === false && encounter.enemy.currentHealth <= 0) ? 0 : Math.round(displayEnemyHp > 0 ? displayEnemyHp : encounter.enemy.currentHealth)) }}/{{ formatNumber(displayEnemyMaxHp > 0 ? displayEnemyMaxHp : (encounter.enemy.stats.maxHealth || 0)) }}</div>
          </div>
        </div>
      </div>
    </div>
...
const enemyHpPct = computed(() => {
  ...
  // 战斗已结束（inProgress=false）：executeRound 同步跑完后 currentHealth 已是终值，
  // 若 enemy 已死则强制归零，避免动画追赶值 displayEnemyHp 残留高血量导致"立绘已弹血条还满"。
  if (enc && enc.inProgress === false && e.currentHealth <= 0) return 0
  // 战斗进行中（inProgress=true）或未开始：走正向展示逻辑，用 displayEnemyHp 随动画推进。
  // 注意：executeRound 在 while 循环中同步跑完所有回合，currentHealth 可能已归零但 inProgress
  // 仍为 true（结算代码在 while 之后才置 false），此时 displayEnemyHp 还在追赶，应让它继续播放
  // 逐回合掉血动画，而不是立即跳到 0——否则玩家会看到"一上来就 0 血"。
