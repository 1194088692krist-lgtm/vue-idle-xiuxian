<template>
  <div class="inventory-page">
    <div class="inventory-tabs">
      ...
      <div class="tab-content">
        ...
        <!-- 宗门 -->
        <div v-if="activeTab === 'sect'" class="tab-pane">
          <div class="sect-header">
            <div class="sect-summary">
              <div class="sect-stat">
                <span class="sect-stat-label">宗门成员</span>
                <span class="sect-stat-value">{{ (playerStore.sectMembers || []).length }} / {{ playerStore.maxSectSize }}</span>
              </div>
              <div class="sect-stat">
                <span class="sect-stat-label">出战队伍</span>
                <span class="sect-stat-value">{{ (playerStore.teamMembers || []).length }} / {{ playerStore.maxTeamSize }}</span>
              </div>
              <div class="sect-stat">
                <span class="sect-stat-label">队伍总战力</span>
                <span class="sect-stat-value gold">{{ playerStore.getTeamTotalBuild() }}</span>
              </div>
            </div>
            <div class="sect-toolbar">
              <select v-model="sectFilter" class="simple-select">
                <option value="all">全部星级</option>
                <option value="5">5星</option>
                <option value="4">4星</option>
                <option value="3">3星</option>
              </select>
              <select v-model="sectSortMode" class="simple-select">
                <option value="score">按评分排序</option>
                <option value="star">按星级排序</option>
                <option value="school">按流派排序</option>
              </select>
              <button class="btn-small btn-primary" @click="autoPickBestTeam">
                ⚡ 自动组建最强队伍
              </button>
            </div>
          </div>
          <div v-if="filteredSectMembers.length" class="sect-grid">
            <div
              v-for="member in filteredSectMembers"
              :key="member.id"
              class="sect-member-card"
              :class="{ 'in-team': isMemberInTeam(member.id) }"
              :style="{ borderColor: getSchoolDisplay(member.school).color }"
            >
              <div class="member-avatar" :style="{ background: getSchoolDisplay(member.school).color + '33' }">
                <img v-if="member.avatar" :src="member.avatar" :alt="member.name" />
                <span v-else>{{ getSchoolDisplay(member.school).icon }}</span>
              </div>
              <div class="member-info">
                <div class="member-name" :style="{ color: starConfig[member.star]?.color || '#fff' }">
                  {{ member.name }}
                </div>
                <div class="member-stars">
                  <span v-for="i in member.star" :key="i" class="star">★</span>
                </div>
                <div class="member-school" :style="{ color: getSchoolDisplay(member.school).color }">
                  {{ getSchoolDisplay(member.school).icon }} {{ getSchoolDisplay(member.school).name }}
                </div>
                <div class="member-meta">
                  <span>Lv.{{ member.level || 1 }}</span>
                  <span class="member-score">战力 {{ playerStore.getCharacterBuildStrength(member) }}</span>
                </div>
              </div>
              <div class="member-actions">
                <button class="btn-small btn-info" @click="showMemberInfo(member)">详情</button>
                <button
                  class="btn-small"
                  :class="isMemberInTeam(member.id) ? 'btn-warning' : 'btn-primary'"
                  @click="toggleMemberInTeam(member.id)"
                >
                  {{ isMemberInTeam(member.id) ? '移出队伍' : '加入队伍' }}
                </button>
                <button class="btn-small btn-danger" @click="confirmRemoveMember(member)">逐出</button>
              </div>
            </div>
          </div>
          <div v-else class="empty-state">
            宗门暂无人手。前往「天机阁」抽卡招募弟子吧！
          </div>
        </div>
      </div>
    </div>
  ...
  <!-- 人物详情弹窗 -->
  <div v-if="showMemberDetail" class="simple-modal" @click.self="closeMemberDetail">
    <div class="simple-modal-content">
      <div class="modal-header">
        <h3>弟子详情</h3>
        <button class="btn-small" @click="closeMemberDetail">关闭</button>
      </div>
      <div v-if="selectedMember" class="modal-body">
        <div class="member-detail-portrait" :style="{ background: getSchoolDisplay(selectedMember.school).color + '33' }">
          <img v-if="selectedMember.avatar" :src="selectedMember.avatar" :alt="selectedMember.name" />
          <span v-else class="member-portrait-placeholder">
            {{ getSchoolDisplay(selectedMember.school).icon }}
          </span>
        </div>
        <div class="member-detail-name" :style="{ color: starConfig[selectedMember.star]?.color || '#fff' }">
          {{ selectedMember.name }}
          <span class="member-stars-inline">
            <span v-for="i in selectedMember.star" :key="i" class="star">★</span>
          </span>
        </div>
        <div class="detail-row">
          <span>流派</span>
          <span :style="{ color: getSchoolDisplay(selectedMember.school).color }">
            {{ getSchoolDisplay(selectedMember.school).icon }} {{ getSchoolDisplay(selectedMember.school).name }}
          </span>
        </div>
        <div class="detail-row">
          <span>天赋</span>
          <span>{{ getTalentDisplay(selectedMember.talent).name }}</span>
        </div>
        <div class="detail-row talent-desc">
          <span></span>
          <span class="talent-desc-text">{{ getTalentDisplay(selectedMember.talent).desc }}</span>
        </div>
        <div class="detail-row">
          <span>等级</span><span>Lv.{{ selectedMember.level || 1 }}</span>
        </div>
        <div class="detail-row">
          <span>战力</span>
          <span class="gold">{{ playerStore.getCharacterBuildStrength(selectedMember) }}</span>
        </div>
        <div class="simple-divider">基础属性</div>
        <div class="detail-row">
          <span>攻击</span><span>{{ selectedMember.baseStats?.attack || 0 }}</span>
        </div>
        <div class="detail-row">
          <span>生命</span><span>{{ selectedMember.baseStats?.health || 0 }}</span>
        </div>
        <div class="detail-row">
          <span>防御</span><span>{{ selectedMember.baseStats?.defense || 0 }}</span>
        </div>
        <div class="detail-row">
          <span>速度</span><span>{{ selectedMember.baseStats?.speed || 0 }}</span>
        </div>
        <div class="member-desc" v-if="selectedMember.description">
          {{ selectedMember.description }}
        </div>
        <div class="modal-actions">
          <button
            class="btn-small"
            :class="isMemberInTeam(selectedMember.id) ? 'btn-warning' : 'btn-primary'"
            @click="toggleMemberInTeam(selectedMember.id)"
          >
            {{ isMemberInTeam(selectedMember.id) ? '移出队伍' : '加入队伍' }}
          </button>
          <button class="btn-small btn-danger" @click="confirmRemoveMember(selectedMember)">逐出宗门</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <!-- 移除确认弹窗 -->
  <div v-if="showRemoveConfirm" class="simple-modal" @click.self="showRemoveConfirm = false">
    <div class="simple-modal-content">
      <div class="modal-header"><h3>确认逐出</h3></div>
      <p>确定要将「{{ memberToRemove?.name }}」逐出宗门吗？此操作不可撤销。</p>
      <div class="modal-actions">
        <button class="btn-small btn-danger" @click="doRemoveMember">确认逐出</button>
        <button class="btn-small" @click="showRemoveConfirm = false">取消</button>
      </div>
    </div>
  </div>
</template>
...
<script setup>
  ...
  const sectSortMode = ref('score') // 'score' | 'star' | 'school'
  const selectedMember = ref(null)
  const showMemberDetail = ref(false)
  const showRemoveConfirm = ref(false)
  const memberToRemove = ref(null)

  const filteredSectMembers = computed(() => {
    let list = [...(playerStore.sectMembers || [])]
    if (sectFilter.value !== 'all') {
      list = list.filter(m => String(m.star) === sectFilter.value)
    }
    if (sectSortMode.value === 'score') {
      list.sort((a, b) => playerStore.getCharacterBuildStrength(b) - playerStore.getCharacterBuildStrength(a))
    } else if (sectSortMode.value === 'star') {
      list.sort((a, b) => b.star - a.star)
    } else if (sectSortMode.value === 'school') {
  ...
  const isMemberInTeam = (memberId) => {
    return (playerStore.teamMembers || []).includes(memberId)
  }

  const toggleMemberInTeam = (memberId) => {
    if (!playerStore.sectMembers) return
    const idx = playerStore.teamMembers.indexOf(memberId)
    if (idx >= 0) {
      playerStore.teamMembers.splice(idx, 1)
      message.success('已移出出战队伍')
    } else {
      if (playerStore.teamMembers.length >= playerStore.maxTeamSize) {
        message.error(`出战队伍最多 ${playerStore.maxTeamSize} 人`)
        return
      }
      playerStore.teamMembers.push(memberId)
      message.success('已加入出战队伍')
    }
    playerStore.queueSave()
  }

  const autoPickBestTeam = () => {
    if (!playerStore.sectMembers || playerStore.sectMembers.length === 0) {
      message.warning('宗门内暂无成员')
      return
    }
    const sorted = [...playerStore.sectMembers].sort(
      (a, b) => playerStore.getCharacterBuildStrength(b) - playerStore.getCharacterBuildStrength(a)
    )
    const best = sorted.slice(0, playerStore.maxTeamSize).map(m => m.id)
    const result = playerStore.setTeamMembers(best)
    if (result.success) {
      message.success(`已自动组建 ${best.length} 人最强队伍`)
    } else {
      message.error(result.message)
    }
  }
  ...
  const doRemoveMember = () => {
    ...
    const result = playerStore.removeSectMember(memberToRemove.value.id)
  ...
  const getSchoolDisplay = (schoolKey) => characterSchools[schoolKey] || { name: schoolKey, color: '#888', icon: '👤' }
  const getTalentDisplay = (talentKey) => characterTalents[talentKey] || { name: talentKey, desc: '' }
