...
// 素材类型展示信息（用于结算栏汇总显示）
const MATERIAL_ORDER = ['herb', 'ore', 'liquid', 'core', 'pet_fragment', 'phantom_crystal', 'boss_material', 'boss_ticket', 'fortune']
...
// BOSS 挑战：消耗对应 BOSS 的挑战券，按当前 Build 与 BOSS 推荐 Build 计算胜率，逐场结算
// zoneId: 秘境 id（如 'forest_edge'）
// bossId: BOSS id（如 'forest_boss_1'，对应 zones.js 中的 bosses[].id）
// count: 挑战次数
// 返回 bossChallengeResult 对象，并同步设置 bossChallengeResult.value
function runBossChallenge(zoneId, bossId, count) {
  const s = store()
  const result = {
    zoneId,
    bossId,
  ...
  // boss 2 略难（+15% build 要求）
  const match = String(bossId || '').match(/_(\d+)$/)
  const idx = match ? Math.max(0, parseInt(match[1], 10) - 1) : 0
  const bossBuild = Math.max(1, Math.floor((diff.recommendedBuild || 1) * (1 + idx * 0.15)))
  const ratio = (s.buildStrength || 1) / bossBuild
  // 胜率公式与 runOfflineEncounter 一致：50% 基础 + Build 比例修正，5%~97% 之间
  const winChance = Math.min(0.97, Math.max(0.05, 0.5 + (ratio - 1) * 0.4))

  // 构建 BOSS 战的 effective zone（用于 grantReward 发放奖励）
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (victory) {
      ...
      const rewards = grantReward(effectiveZone, false, true)
      ...
      result.defeats++
      // 失败损失少量修为池（参考 runOfflineEncounter）
      const loss = Math.floor((s.cultivationPool || 0) * 0.05)
